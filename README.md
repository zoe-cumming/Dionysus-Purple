# Dionysus-Purple
CSSE4011 Project - Couch Potato Paradise: Smart Home System Using Sensor Data

## Team Contribution Table
---

| Team Member      | Description of Contribution                                                             |
|------------------|-----------------------------------------------------------------------------------------|
| Tara Bashirzadeh | Machine learning model development. Two-way bluetooth communication between the Thingy52 and nRF52840 DK. Temperature sensor set up.  |
| Zoe Cumming      | Collecting and streaming image data using ESP32-S3-EYE (via TCP socket). Wifi set-up. DMIC and light sensor set up. Configuring servo motor. |
| Jacob Gregg      | MQTT set-up. Dashboard and M5 Core 2 displays. Configuring servo motor. |

---

## Equipment
- **nRF52840 DK** (Base node, nanoPB encoder)
- **Thingy:52** (Audio, light and temperature sensor node)
- **ESP32C3 DevkitM** (Servo controller)
- **Disco L475 IOT01** (Servo controller)
- **M5Core2** (Display)
- **ESP32 S3 EYE** (Image collection node)
- **PC** (ML implementation, MQTT comms, nanoPB decoder)
- **MQTT Server** (Sensor and setting data transmission)
